...
from .experiment import get_next_filename
import pandas as pd
import pyttsx3
...
def infer_columns(row):
    ...
    result = pd.Series([touristic, children, romantic, assigned_seats], index=["touristic", "children", "romantic", "assigned seats"])
...
class SystemMessageGenerator:
    ...
    def _print_proposition(self):
        ...
        if add:
            ...
            if key == "touristic" and self.recommendation[key]:
                contradiction= f" even though the food is romanian," if self.recommendation["food"]=="romanian"  else ""
        ...
        foodtype_str= self.recommendation['food'] if 'food' in self.recommendation['food'] else f"{self.recommendation['food']} food"
        # system's response
    ...
    def _fit_propositions(self):
        ...
        """
        self.propositions.loc[:,["touristic", "children", "romantic", "assigned seats"]] = self.propositions.apply(infer_columns, axis=1)
    ...
    def generate_next_step(self, predicted_intent, user_input, state=0, restart=False):
        ...
        if self.curr_state==3:
            ...
            if not all_info_retrieved(self.additional_dict):
                ...
                self.additional_dict.update({"touristic": "dontcare", "romantic": "dontcare", "children": "dontcare", "assigned seats": "dontcare"})
            else:
        ...
        if self.curr_state==4: 
            ...
            # if they ask for alternatives
            elif predicted_intent in ["reqmore" , "reqalt"]:
                if self.candidates is not None and self.candidates.shape[0]>1:
                    ...
                    for i, row in self.candidates.iloc[1:].iterrows():
                        string = f"{row['restaurantname']} is in the {row['area']} part of town and it serves {row['food']} food."
